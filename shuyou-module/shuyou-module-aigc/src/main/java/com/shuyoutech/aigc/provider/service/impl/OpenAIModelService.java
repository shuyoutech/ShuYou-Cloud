...
public class OpenAIModelService implements ModelService {
    ...
    /**
     * <a href="https://platform.openai.com/docs/api-reference/introduction"></a>
     */
    @Override
    public void chat(ChatModelBuilder builder, HttpServletResponse response) {
        try {
            ...
            log.info("chat ============================ OpenAI request:{}", requestBody);
            ...
            if (!await) {
                log.error("chat OpenAI ============================ getCountDownLatch timed out");
            }
        } catch (Exception e) {
            log.error("chat OpenAI ===================== exception:{}", e.getMessage());
        }
    }
    ...
    private void audioSpeech(CommonModelBuilder builder, HttpServletResponse response) {
        try {
            ...
            Response res = client.newCall(request).execute();
            if (res.isSuccessful()) {
                RemoteSysFile file = remoteSystemService.upload(IdUtil.fastSimpleUUID() + "." + responseFormat, res.body().bytes());
                ...
                PrintWriter writer = response.getWriter();
                writer.write(JSON.toJSONString(R.success(JSON.toJSONString(file))));
                writer.flush();
            } else {
                response.setStatus(res.code());
                PrintWriter writer = response.getWriter();
