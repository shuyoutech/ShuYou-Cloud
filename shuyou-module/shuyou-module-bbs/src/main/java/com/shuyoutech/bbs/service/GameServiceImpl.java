...
public class GameServiceImpl extends SuperServiceImpl<GameEntity, GameVo> implements GameService {
    ...
    public Query buildQuery(GameBo bo) {
        ...
        if (null != bo.getBeenHotGame()) {
            query.addCriteria(Criteria.where("beenHotGame").is(bo.getBeenHotGame()));
        }
        return query;
    }
    ...
    public Map<String, Object> querySearch(GameBo bo) {
        ...
        List<String> goodsTypeList = game.getGoodsTypeList();
        result.put("goodsType", goodsTypeList);

        List<String> crossAreaList = game.getCrossAreaList();
        result.put("crossArea", crossAreaList);

        List<GameAreaServer> gameAreaList = game.getAreaServerList();
        result.put("areaServer", gameAreaList);

        List<String> gameRaceList = game.getGameRaceList();
        result.put("gameRace", gameRaceList);

        List<String> gameRoleList = game.getGameRoleList();
        result.put("gameRole", gameRoleList);

        List<String> shopIdList = game.getShopIdList();
        if (CollectionUtils.isNotEmpty(shopIdList)) {
            List<GameShopEntity> platforms = MongoUtils.getByIds(shopIdList, GameShopEntity.class);
            List<DropDownOptions> platformsList = CollectionUtils.newArrayList();
            if (CollectionUtils.isNotEmpty(platforms)) {
                for (GameShopEntity platform : platforms) {
                    platformsList.add(new DropDownOptions().value(platform.getId()).label(platform.getShopAlias()).sort(1));
                }
            }
    ...
    public List<GameGoodsUnitPriceVo> latestUnitPrice(String gameId) {
        ...
        for (GameAreaServer areaServer : game.getAreaServerList()) {
            ...
            // 是否有种族分区
            if (CollectionUtils.isEmpty(game.getGameRaceList())) {
                ...
                for (String gameRace : game.getGameRaceList()) {
